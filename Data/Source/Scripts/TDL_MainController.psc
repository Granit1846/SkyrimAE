...
; =====================================================
; PROPERTIES (прив€зать в Creation Kit)
; =====================================================
GlobalVariable Property TDL_DebugEnabled Auto
GlobalVariable Property TDL_LastErrorCode Auto
Spell Property TDL_MenuPower Auto  ; < Lesser Power из CK

; =====================================================
; INIT Ч автоматическа€ регистраци€ способности
; =====================================================
Event OnInit()
    Actor player = Game.GetPlayer()
    If player && TDL_MenuPower && !player.HasSpell(TDL_MenuPower)
        player.AddSpell(TDL_MenuPower)
        ; Debug.Notification("TDL Menu Power added")
    EndIf
EndEvent

; =====================================================
; ENTRY POINT Ч вызываетс€ через SetStage(aiStage)
; =====================================================
Bool Function RunStage(Int aiStage)
	...
	; ===== MENU TRIGGER =====
	ElseIf aiStage == 999
  	  ; —игнал дл€ DLL: открыть кастомное меню
   	  Return True
